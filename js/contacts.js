const ContactManager = (() => {
  ...
  function renderDashboard() {
    ...
    if (dashboardEmptyState) dashboardEmptyState.hidden = total !== 0;
    ...
    el.recentList.innerHTML = recent.length
      ? recent.map((c) => `
        <div class="recent-item" data-id="${c.id}" role="button" tabindex="0">
          <div class="recent-avatar">${Validation.isSafeImageDataUrl(c.photo) ? `<img src="${c.photo}" alt="Photo of ${Utils.escapeHtml(c.fullName || 'contact')}" />` : Utils.escapeHtml(Utils.initials(c.fullName))}</div>
    ...
    el.recentList.querySelectorAll('.recent-item').forEach((node) => {
      ...
      node.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); openDetailModal(node.dataset.id); } });
    });
  ...
  function renderCardDeck() {
    ...
    const emptyAdd = document.getElementById('btnEmptyAddContact');
    const hasContacts = contacts.length > 0;
    if (emptyTitle) emptyTitle.textContent = hasContacts ? 'No contacts match your filters' : 'No contacts yet';
    if (emptySub) emptySub.textContent = hasContacts
      ? 'Try adjusting your search terms or filters.'
      : 'Add your first contact to get started.';
    if (emptyAdd) emptyAdd.hidden = hasContacts;
    ...
    el.cardDeck.querySelectorAll('.bcard').forEach((node) => {
      ...
      // Front face click — flip in grid, open detail in list
      const frontFace = node.querySelector('.bcard-face:not(.is-back)');
      ...
      // Back face click — flip back (unless a button was clicked)
      ...
      // All [data-action] buttons (back face + list-mode)
      node.querySelectorAll('[data-action]').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          ...
          if (action === 'delete') requestDeleteContact(id);
        });
      });
    });
  }
  ...
  function cardTemplate(c) {
    const avatar = Validation.isSafeImageDataUrl(c.photo)
      ? `<img src="${c.photo}" alt="Photo of ${Utils.escapeHtml(c.fullName || 'contact')}" />`
      : Utils.escapeHtml(Utils.initials(c.fullName));

    const priorityDotClass = c.priority || 'medium';
    const emailDisplay = c.email ? Utils.escapeHtml(c.email) : '<span class="bcard-back-row-empty">—</span>';
    const phoneDisplay = c.phone ? Utils.escapeHtml(c.phone) : '<span class="bcard-back-row-empty">—</span>';
    const webDisplay   = c.website ? Utils.escapeHtml(c.website.replace(/^https?:\/\//, '')) : '<span class="bcard-back-row-empty">—</span>';

    return `
      <div class="bcard" data-id="${c.id}" role="listitem">
        <div class="bcard-inner">

          <!-- FRONT -->
          <div class="bcard-face">
            <div class="bcard-front-inner">
              <div class="bcard-top">
                <div class="bcard-avatar">${avatar}</div>
                <button class="bcard-fav${c.favorite ? ' is-fav' : ''}"
                  ...
                  <svg aria-hidden="true"><use href="#icon-star-${c.favorite ? 'filled' : 'empty'}"/></svg>
                </button>
              </div>
              ...
              <div class="bcard-bottom">
                ...
                <span class="bcard-hint">
                  <svg aria-hidden="true"><use href="#icon-flip"/></svg>
                  flip
                </span>
              </div>
            </div>
            <!-- List-mode quick actions (hidden in grid) -->
            <div class="bcard-list-actions">
              <button type="button" class="bcard-list-btn" data-action="view" title="View details" aria-label="View ${Utils.escapeHtml(c.fullName || 'contact')}">
                <svg aria-hidden="true"><use href="#icon-eye"/></svg>
              </button>
              <button type="button" class="bcard-list-btn" data-action="edit" title="Edit" aria-label="Edit ${Utils.escapeHtml(c.fullName || 'contact')}">
                <svg aria-hidden="true"><use href="#icon-edit"/></svg>
              </button>
              <button type="button" class="bcard-list-btn btn-del" data-action="delete" title="Delete" aria-label="Delete ${Utils.escapeHtml(c.fullName || 'contact')}">
          ...
          <div class="bcard-face is-back">
            <div class="bcard-back-inner">
              <div class="bcard-back-header">
                <span class="bcard-back-name">${Utils.escapeHtml(c.fullName || 'Untitled')}</span>
                <span class="bcard-back-flip-hint">flip back</span>
              </div>
              <div class="bcard-back-fields">
                <div class="bcard-back-row">
                  <svg class="bcard-back-row-icon" aria-hidden="true"><use href="#icon-email"/></svg>
                  ${emailDisplay}
                </div>
                <div class="bcard-back-row">
                  <svg class="bcard-back-row-icon" aria-hidden="true"><use href="#icon-phone"/></svg>
                  ${phoneDisplay}
                </div>
                <div class="bcard-back-row">
                  <svg class="bcard-back-row-icon" aria-hidden="true"><use href="#icon-globe"/></svg>
                  ${webDisplay}
                </div>
              </div>
              <div class="bcard-back-actions">
                <button type="button" class="bcard-icon-btn" data-action="view">
                  <svg aria-hidden="true"><use href="#icon-eye"/></svg>
                  View
                </button>
                <button type="button" class="bcard-icon-btn" data-action="edit">
                  <svg aria-hidden="true"><use href="#icon-edit"/></svg>
                  Edit
                </button>
                <button type="button" class="bcard-icon-btn btn-del" data-action="delete">
                  <svg aria-hidden="true"><use href="#icon-trash"/></svg>
                  Delete
                </button>
              </div>
  ...
  function renderCategories() {
    ...
    el.categoryList.innerHTML = categories.length
      ? categories.map((cat) => `
        <div class="category-tile" data-category="${Utils.escapeHtml(cat)}" role="listitem">
          ...
          <button class="category-tile-remove" title="Remove category" aria-label="Remove category ${Utils.escapeHtml(cat)}" type="button">
            <svg aria-hidden="true"><use href="#icon-trash"/></svg>
          </button>
        </div>`).join('')
  ...
  function requestDeleteCategory(name) {
    ...
    if (title) title.textContent = 'Remove category';
  ...
  function resetForm() {
    ...
    const formErrorText = document.getElementById('contactFormErrorText');
  ...
  // New business-management fields
    ...
    if (firstInvalidField) {
      ...
      formErrorText.textContent = errors[firstInvalidField];
    ...
    if (duplicate) {
      formErrorText.textContent = `Possible duplicate: ${duplicate.fullName}${duplicate.company ? ` at ${duplicate.company}` : ''}. Review the existing contact before saving.`;
      formError.hidden = false;
      el.fieldFullName.focus();
      return;
    }
  ...
  function requestDeleteContact(id) {
    ...
    const title = document.getElementById('confirmTitle');
    if (title) title.textContent = 'Delete contact';
  ...
  function openDetailModal(id) {
    const c = contacts.find((x) => x.id === id);
    if (!c) {
      Utils.showToast('Contact not found.', 'error');
      closeDetailModal();
      return;
    }
    detailContactId = id;
    ...
    if (Validation.isSafeImageDataUrl(c.photo)) {
      el.detailPhoto.innerHTML = `<img src="${c.photo}" alt="Photo of ${Utils.escapeHtml(c.fullName || 'contact')}" />`;
    } else {
    ...
    // Badge row: category + priority + status
    const badgeRow = document.getElementById('detailBadgeRow');
    const priorityClass = { high: 'badge-priority-high', medium: 'badge-priority-medium', low: 'badge-priority-low' };
    const statusClass   = { active: 'badge-status-active', 'follow-up': 'badge-status-follow-up', inactive: 'badge-status-inactive' };

    badgeRow.innerHTML = `
      <span class="badge">${Utils.escapeHtml(c.category || 'Uncategorized')}</span>
      ${c.priority ? `<span class="badge ${priorityClass[c.priority] || ''}">${Utils.escapeHtml(c.priority)}</span>` : ''}
      ${c.status   ? `<span class="badge ${statusClass[c.status]   || ''}">${Utils.escapeHtml(c.status)}</span>`   : ''}
    `;
    ...
    const rows = [
      ...
      ['Next follow-up',c.nextFollowUpAt  ? Utils.formatDate(c.nextFollowUpAt)  : null],
      ['Added',         Utils.formatDate(c.createdAt)],
      ['Last updated',  c.updatedAt ? Utils.formatDate(c.updatedAt) : null],
    ].filter(([, v]) => v);
